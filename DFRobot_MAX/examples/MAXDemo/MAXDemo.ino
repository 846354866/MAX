...
void avoidObstacle()
{
	static uint32_t musicTime, emoticonsTime, RGBTime;
	uint8_t flag, randomNum=1;
	...
	if(1) {
		if(getValue > 105) {
			...
			if(randomNum == 1) {
				myMax.swerve(ADVANCE_L, 200, ADVANCE_R, 100); //turn right
			} else if(randomNum == 2) {
				myMax.swerve(ADVANCE_L, 100, ADVANCE_R, 200); //turn left
			} else if(randomNum == 3) {
				myMax.swerve(RETREAT_L, 200, RETREAT_R, 100); //retreat left deflection
			} else {
				myMax.swerve(RETREAT_L, 100, RETREAT_R, 200); //retreat right deflection
			}
		} else {
			myMax.retreat(150, 150);
		}
	}
...
void linePatrol()
{
	...
	static uint8_t minSpeed = 50, speed_R, speed_L, musicNum=1, flag;
...
void rayTracing()
{
	static uint32_t emoticonsTime, RGBTime;
